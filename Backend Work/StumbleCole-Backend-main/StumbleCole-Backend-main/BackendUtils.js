...
class UserModel {
  ...
  static async create(deviceId, platformData = {}) {
    ...
    const user = {
      ...
      // Sistema de sufixo [B] para Booster
      bSuffix: {
        enabled: false,
        color: null // Cor do booster do Discord
      },
      // Campo de troféus
      trophy: 0
    };
  ...
  // Função para migrar usernames de sgmasters para sgzone
  static async migrateUsernamesFromMastersToZone() {
    try {
      ...
      Console.log("Migration", `Migrados ${result.modifiedCount} usuários de sgmasters para sgzone`);
      return result;
    } catch (err) {
      Console.error("Migration", `Erro ao migrar usernames: ${err.message}`);
      throw err;
    }
  }
  ...
  // Aplica o sufixo [B] ao username se estiver habilitado
  static applyBSuffix(username, bSuffix) {
    if (!bSuffix || !bSuffix.enabled || !bSuffix.color) {
      return username;
    }
    
    // Remove qualquer [B] existente primeiro
    let cleanUsername = username.replace(/<color=#[0-9a-fA-F]{6}>\[B\]<\/color>/g, '').trim();
    
    // Adiciona o novo [B] colorido
    return `${cleanUsername}<color=#${bSuffix.color}>[B]</color>`;
  }

  // Remove o sufixo [B] do username
  static removeBSuffix(username) {
    return username.replace(/<color=#[0-9a-fA-F]{6}>\[B\]<\/color>/g, '').trim();
  }

  // Habilita o sufixo [B] para um usuário
  static async enableBSuffix(stumbleId, hexColor) {
    const user = await this.findByStumbleId(stumbleId);
    if (!user) {
      throw new Error('User not found');
    }

    // Remove [B] antigo do username
    const cleanUsername = this.removeBSuffix(user.username);
    
    // Aplica o novo [B] com a cor
    const newUsername = `${cleanUsername}<color=#${hexColor}>[B]</color>`;
    ...
    await database.updateUser(
      ...
      {
        username: newUsername,
        'bSuffix.enabled': true,
        'bSuffix.color': hexColor
      }
    );
  ...
  // Desabilita o sufixo [B] para um usuário
  static async disableBSuffix(stumbleId) {
    const user = await this.findByStumbleId(stumbleId);
    if (!user) {
      throw new Error('User not found');
    }

    // Remove [B] do username
    const cleanUsername = this.removeBSuffix(user.username);
  ...
  static tournaments = [
    {
      ...
      headerColour: "#007799ff",
      mapListGradientColourTop: "#004466ff",
      mapListGradientColourBottom: "#002233ff",
      listPriority: 1,
      minPlayers: 2,
      maxPlayers: 2,
      maxRounds: 1,
      ...
      detailsPanelBorderColourTop: "#004080ff",
      detailsPanelBorderColourBottom: "#002244ff",
      colourData: {
        detailsPanelMainColour: "#003366ff",
        detailsPanelBorderColour: "#004080ff",
        headerGradientRight: "#003366ff",
        headerGradientLeft: "#005599ff",
        infoWidgetsGradientRight: "#003366ff",
        infoWidgetsGradientLeft: "#002244ff",
        infoWidgetsBorderColour: "#004080ff"
      },
      rounds: [
        {
          roundOrder: 1,
    ...
    },
    {
      ...
      headerColour: "#ff69b4ff",
      mapListGradientColourTop: "#1e90ffff",
      mapListGradientColourBottom: "#4169e1ff",
      listPriority: 1,
      minPlayers: 2,
      maxPlayers: 2,
      maxRounds: 1,
      ...
      areEmotesRestricted: false,
      prohibitedEmotes: [8, 13, 55, 122, 123, 124],
      detailsPanelBorderColourTop: "#00ff00ff",
      detailsPanelBorderColourBottom: "#32cd32ff",
      colourData: {
        detailsPanelMainColour: "#1e90ffff",
        detailsPanelBorderColour: "#00ff00ff",
        headerGradientRight: "#ff1493ff",
        headerGradientLeft: "#ff69b4ff",
        infoWidgetsGradientRight: "#4169e1ff",
        infoWidgetsGradientLeft: "#1e90ffff",
        infoWidgetsBorderColour: "#00ff00ff"
      },
      rounds: [
        {
          roundOrder: 1,
